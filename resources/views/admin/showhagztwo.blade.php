...
<table class="table">
  ...
  @foreach($hagzs as $index=>$hagz)
    <tr>
      ...
      <td>{{$hagz->note}}</td>
      <td scope="row"><a class="btn btn-danger" onclick="return confirm('هل انت متاكد من الحذف?')" href="{{route('admin.showhagztwo.notshowadmintwo',['id' => $hagz->id])}}" ><i class="fas fa-trash"></i></a></td>
